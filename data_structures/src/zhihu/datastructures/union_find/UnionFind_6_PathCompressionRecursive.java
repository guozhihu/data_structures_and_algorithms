...
/**
 * Author: zhihu
 * Description: 并查集的简单实现-基于对应元素所在根的集合的树的层数进行优化和路径压缩(采用递归进行路径压缩，树的层数会变得最少，但效率不一定高),实现了Quick Find和Quick Union
 * Quick Find的时间复杂度为O(h),h为树的高度
 * Quick Union的时间复杂度为O(h),h为树的高度
 * Date: Create in 2019/1/27 23:50
 */
public class UnionFind_6_PathCompressionRecursive implements IUnionFind {
    ...
    public UnionFind_6_PathCompressionRecursive(int size) {
        parent = new int[size];
        rank = new int[size];
